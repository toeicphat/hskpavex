...
export const LogoIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" {...props}>
        <defs>
            {/* Base gradient for the icon color */}
            <linearGradient id="logoBaseGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#93c5fd" /> {/* blue-300 */}
                <stop offset="100%" stopColor="#67e8f9" /> {/* cyan-300 */}
            </linearGradient>

            {/* Define the paths as a reusable symbol */}
            <symbol id="logoShape">
                <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5-10-5-10 5zM2 12l10 5 10-5-10-5-10 5z" opacity="0.8"/>
                <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5-10-5-10 5z"/>
            </symbol>

            {/* Gradient for the moving shine effect */}
            <linearGradient id="shineAnimatedGradient" x1="0" y1="0" x2="20" y2="0"> {/* x2 > x1 to define width of shine */}
                <stop offset="0%" stopColor="rgba(255,255,255,0)" />
                <stop offset="30%" stopColor="rgba(255,255,255,0.7)" /> {/* Bright part of the shine */}
                <stop offset="70%" stopColor="rgba(255,255,255,0.7)" />
                <stop offset="100%" stopColor="rgba(255,255,255,0)" />
                {/* Animate x1 and x2 to move the gradient across the viewBox */}
                <animate attributeName="x1" from="-24" to="24" dur="2s" repeatCount="indefinite" /> {/* Start off-left, move to right */}
                <animate attributeName="x2" from="0" to="48" dur="2s" repeatCount="indefinite" />   {/* End off-left, move to right, maintaining width */}
            </linearGradient>

            {/* Mask to apply the shine only to the logo shape */}
            <mask id="logoMask">
                <use href="#logoShape" fill="white" /> {/* Use white to make the mask fully opaque where shape is */}
            </mask>
        </defs>

        {/* Base icon layer with its main gradient fill */}
        <use href="#logoShape" fill="url(#logoBaseGradient)" />

        {/* Shine layer: a rectangle filled with the animated gradient, masked by the logo shape */}
        {/* The rect covers the whole viewBox, and the animated fill combined with the mask creates the shine effect */}
        <rect x="0" y="0" width="24" height="24" fill="url(#shineAnimatedGradient)" mask="url(#logoMask)" />
    </svg>
);
